...
class FeatureShow():
    ...
    def update_widgets(self, resize=False):
        ...
        if self.stat_stat_info:
            ...
            'weights_keys: '+ str(self.recordnode.weights.keys()) + '\n' + \
            'weights_keys: '+ str([type(self.recordnode.weights[k]) for k in self.recordnode.weights.keys() if (self.recordnode.weights[k] is not None)]) + '\n' + \
            'weights_keys: '+ str([self.recordnode.weights[k].shape for k in self.recordnode.weights.keys() if (self.recordnode.weights[k] is not None)]) + '\n' + \
            'Your Majesty.'
        if self.stat_parent_function:
            tstat_str += '\n\n' + self.parent_net
        if self.stat_function:
